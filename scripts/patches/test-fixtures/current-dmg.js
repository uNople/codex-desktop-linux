...
function currentChromePluginAppServerSourceBundleFixture() {
  return [
    ...
    "async function cq(e){let t=[...new Set([...e.extensionIds,...nb(e.nativeHostName)])],n=Aq(),r=await kq({pluginRoot:e.pluginRoot,target:n});return{browserClientPath:i.join(e.pluginRoot,`scripts`,`browser-client.mjs`),extensionIds:t,target:n,extensionHostPath:r}}",
    "function nb(){return[]}function Aq(){return{platform:process.platform,architecture:process.arch,filename:`extension-host`}}async function kq(e){return i.join(e.pluginRoot,`extension-host`,e.target.platform,e.target.architecture,e.target.filename)}",
